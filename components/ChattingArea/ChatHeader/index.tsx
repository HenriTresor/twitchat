...
function ChatHeader({ }: Props) {
    ...
    const { currentChat, currentUser } = useContext(AppData)
    return (
        <div className='w-full p-2 flex justify-between  border-b-2 h-auto border-slate-100 sticky top-0 z-50 backdrop-blur-sm bg-[rgb(0,0,0,.01)] items-end'>
            <div className='flex items-center gap-[2em]'>
                ...
                <div>
                    <h1 className='font-extrabold tracking-wider'>{currentChat?.users[0]?._id !== currentUser?._id && currentChat?.users[0]?.names}</h1>
